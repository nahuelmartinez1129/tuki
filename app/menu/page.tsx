...
export default function MenuPage() {
  ...
  const [category, setCategory] = useState<MenuCategoryGroup>("todos");

  const topVentasIds = useMemo(
    () =>
      new Set(
        allProducts
          .filter((product) => product.tags?.includes("top-ventas"))
  ...
  const filteredProducts = useMemo(() => {
    ...
    return allProducts.filter((product) => {
      const matchesCategory =
        category === "todos" || getMenuGroup(product) === category;
      const matchesQuery =
